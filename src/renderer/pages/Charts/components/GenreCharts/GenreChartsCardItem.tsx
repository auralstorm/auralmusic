...
const GenreChartsCardItem = ({ chart, onOpen }: GenreChartsCardProps) => {
  const statLabel =
    chart.trackCount > 0 ? `${chart.trackCount}首` : (chart.updateFrequency ?? '榜单')

  return (
    <div className='group flex flex-col gap-3 text-left' onClick={() => onOpen(chart.id)}>
      <div className='relative aspect-square overflow-hidden rounded-[14px] border border-border/60 bg-muted shadow-[0_18px_44px_rgba(15,23,42,0.14)] transition-transform duration-300 group-hover:-translate-y-1 group-hover:-translate-x-1 group-hover:shadow-[10px_10px_20px_rgba(0,0,0,0.2)]'>
        {chart.coverImgUrl ? (
          <div
        ...
        <div className='relative z-10 flex h-full flex-col justify-between p-4'>
          ...
          <div className='text-xs text-white/60 group-hover:text-white/100 transition-colors duration-300'>
      ...
      <div className='space-y-1 px-0.5'>
        ...
        <p className='line-clamp-1 text-sm text-muted-foreground'>{chart.description}</p>
      </div>
    </div>
  )
}
